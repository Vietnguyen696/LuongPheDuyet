...
import { X, CheckCircle, XCircle, User, Briefcase, MessageSquare, Calendar, Award } from 'lucide-react';
import { RecordItem, Status, ViewMode, PartyMember, ApprovalInfo } from '../types';
import { GRADING_OPTIONS } from '../constants';
...
export const GradingModal: React.FC<GradingModalProps> = ({ 
  ...
  // Form State (View Only mostly, except creating)
  const [formData, setFormData] = useState({
      gradingYear: record?.gradingYear || new Date().getFullYear().toString(),
      selfGrading: record?.selfGrading || GRADING_OPTIONS[1],
  ...
  const handleConfirm = () => {
      if (canAct && onApprove) {
          ...
          // Pass the grade back to App.tsx
          const payload = isLevel1Approval ? { proposedGrading: approvalGrade } : { finalGrading: approvalGrade };
          onApprove(record!.id, comment, payload);
      }
  };
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
          <h3 className="text-lg font-bold text-gray-800 uppercase tracking-wide">
             {isCreate ? "Đăng ký Xếp loại Đảng viên" : "Chi tiết Xếp loại Đảng viên"}
          </h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
             {/* Left Column: Member Info & Self Grading */}
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                        ...
                        <input type="text" className={readOnlyClass} value={record?.targetMemberName || "Nguyễn Văn A"} disabled />
                    </div>
                    <div>
                        <label className={labelClass}>Mã Đảng viên</label>
                        <input type="text" className={readOnlyClass} value={record?.targetMemberCode || "DV-001"} disabled />
                    </div>
                    <div>
                        ...
                        <div>
                            ...
                            {isCreate ? (
                                <select className={inputClass} value={formData.selfGrading} onChange={(e) => setFormData({...formData, selfGrading: e.target.value})}>
                                    {GRADING_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                </select>
                            ) : (
             ...
             <div className="space-y-6">
                ...
                {canAct && (
                    <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100 shadow-sm">
                        <h4 className="text-sm font-bold text-blue-800 mb-4 flex items-center">
                            ...
                            {isLevel1Approval ? "Thẩm định & Đề xuất (Cấp 1)" : "Phê duyệt (Cấp 2)"}
                        </h4>
                        
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {isLevel1Approval ? "Đề xuất xếp loại" : "Phê duyệt xếp loại"} <span className="text-red-500">*</span>
                                </label>
                                <select 
                ...
                {(record?.level1Result || record?.level2Result) && (
                    <div>
                        ...
                        <ApprovalHistoryItem title="Phê duyệt Cấp 1" info={record?.level1Result} grade={record?.proposedGrading} />
                        <ApprovalHistoryItem title="Phê duyệt Cấp 2" info={record?.level2Result} grade={record?.finalGrading} />
                    </div>
                )}
